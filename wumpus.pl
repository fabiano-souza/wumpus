...
% Seleciona uma célula disponível aleatória
% e exclui esta célula da lista de células disponíveis:
seleciona_celula(X) :-
...
avancar(T) :-
	...
	% Checa por parede e retorna o agente caso encontre,
	% e checa por buraco ou wumpus, que matam o agente:
	(
		parede(Z),
		percepcoes(_),
		retractall(agente(_)),
...
% Avança o que quer que seja
% (POSIÇÃO INICIAL, DIREÇÃO INICIAL, POSIÇÃO FINAL):
avancar(A,B,C) :-
	A = [X,Y],
	(
